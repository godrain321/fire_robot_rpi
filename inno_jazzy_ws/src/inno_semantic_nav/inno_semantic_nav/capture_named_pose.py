"""Capture one RViz 2D Goal Pose and store it under a semantic name."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='RViz 2D Goal Pose를 이름 있는 pose로 저장합니다.')
    parser.add_argument('name', help='저장할 pose 이름')
    parser.add_argument('--semantic-file', required=True, help='semantic_points.yaml 경로')
    parser.add_argument('--category', default='', help='분류(예: exit)')
    parser.add_argument('--description', default='', help='설명')
    parser.add_argument('--timeout', type=float, default=120.0, help='수신 대기시간(초)')
    parser.add_argument('--overwrite', action='store_true', help='같은 이름의 pose를 덮어쓰기')
    return parser


def _wait_for_pose(node: Node, timeout: float) -> Optional[PoseStamped]:
    received = []
    ...
    subscription = node.create_subscription(PoseStamped, '/goal_pose', callback, 10)
...
def main(args=None) -> None:
    ...
    try:
        ...
        if parsed.name in existing['landmarks']:
            raise DuplicateNameError(
                f'{parsed.name!r}은(는) 이미 landmark 이름으로 사용 중입니다.'
            )
        if parsed.name in existing['poses'] and not parsed.overwrite:
            raise DuplicateNameError(
    ...
    node = Node('capture_named_pose')
    try:
        print('RViz에서 2D Goal Pose를 선택한 뒤 저장할 위치와 방향을 클릭하십시오.')
        message = _wait_for_pose(node, parsed.timeout)
        if message is None:
            print(f'오류: /goal_pose를 {parsed.timeout:g}초 안에 받지 못했습니다.', file=sys.stderr)
            return
        ...
        if message.header.frame_id != expected_frame:
            print(
                ...
                f'{expected_frame!r} frame의 목표만 저장할 수 있습니다.',
                file=sys.stderr,
            )
            return
        ...
        print(
            f'{parsed.name} 저장 완료: x={message.pose.position.x:.6f}, '
            f'y={message.pose.position.y:.6f}, yaw={yaw:.6f} rad\n'
            f'파일: {store.path}'
        )
